...
from sqlalchemy import create_session, engine_from_config
...
async def start_campaign(player_id: str):
    db = SessionLocal()
    ...
    db.commit()
    db.close()
    return {"campaign_id": campaign_id, "start_hex": 1}
...
async def process_chat_action(request: Request):
    ...
    response = {
        ...
        "visual_assets": final_output.get("visual_assets", {}) # Added
    }
