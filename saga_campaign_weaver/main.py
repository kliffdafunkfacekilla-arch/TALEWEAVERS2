...
async def create_regional_arc(
    saga_beat: dict,
    region_context: dict,
    context_packet: Optional[dict] = None
):
    try:
        return await generate_regional_arc(
            saga_beat, region_context, context_packet)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))


@app.post("/api/weaver/sidequest", response_model=QuestNode)
async def create_sidequest(
    hex_context: dict,
    context_packet: Optional[dict] = None
):
    try:
        return await generate_local_sidequest(
            hex_context, context_packet)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def place_poi(request: POIRequest):
    try:
        ...
        placer = POIPlacer()
        result = placer.place_node_on_grid(
            request.quest_node, request.grid_data)
        return result
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
